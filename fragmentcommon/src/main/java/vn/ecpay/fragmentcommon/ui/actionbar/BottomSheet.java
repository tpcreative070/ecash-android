...
import vn.ecpay.fragmentcommon.R;
...
public class BottomSheet extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (containerView == null) {
            ...
            containerView.setPadding(backgroundPaddingLeft, (applyTopPadding ? Utilities.dp(8) : 0) + backgroundPaddingTop, backgroundPaddingLeft, (applyBottomPadding ? Utilities.dp(8) : 0));
        }
        if (Build.VERSION.SDK_INT >= 21) {
        ...
        if (customView != null) {
            ...
            if (title != null) {
                ...
                titleView.setPadding(Utilities.dp(16), 0, Utilities.dp(16), Utilities.dp(8));
    ...
    public void show() {
        ...
        if (containerView.getMeasuredHeight() == 0) {
            containerView.measure(View.MeasureSpec.makeMeasureSpec(Utilities.displaySize.x, View.MeasureSpec.AT_MOST), View.MeasureSpec.makeMeasureSpec(Utilities.displaySize.y, View.MeasureSpec.AT_MOST));
        }
        backDrawable.setAlpha(0);
        if (Build.VERSION.SDK_INT >= 18) {
            layoutCount = 2;
            Utilities.runOnUIThread(startAnimationRunnable = new Runnable() {
                @Override
                public void run() {
    ...
    public void dismissWithButtonClick(final int item) {
        ...
        AnimatorSet animatorSet = new AnimatorSet();
        animatorSet.playTogether(
                ObjectAnimator.ofFloat(containerView, "translationY", containerView.getMeasuredHeight() + Utilities.dp(10)),
    ...
    public static class BottomSheetCell extends FrameLayout {
        ...
        public BottomSheetCell(Context context, int type) {
            ...
            setPadding(Utilities.dp(16), 0, Utilities.dp(16), 0);
        ...
        public void setTextAndIcon(CharSequence text, int icon) {
            ...
            if (icon != 0) {
                ...
                textView.setPadding(/*LocaleController.isRTL ? 0 :*/ Utilities.dp(56), 0, /*LocaleController.isRTL ? Utilities.dp(56) :*/ 0, 0);
            } else {
    ...
    private class ContainerView extends FrameLayout implements NestedScrollingParent {
        ...
        private void checkDismiss(float velX, float velY) {
            ...
            boolean backAnimation = translationY < Utilities.getPixelsInCM(0.8f, false) && (velY < 3500 || Math.abs(velY) < Math.abs(velX)) || velY < 0 && Math.abs(velY) >= 3500;
            if (!backAnimation) {
                ...
                currentAnimation.setDuration((int) (150 * (translationY / Utilities.getPixelsInCM(0.8f, false))));
            ...
            if (containerView != null) {
                if (!fullWidth) {
                    ...
                    if (Utilities.isTablet()) {
                        widthSpec = MeasureSpec.makeMeasureSpec((int) (Math.min(Utilities.displaySize.x, Utilities.displaySize.y) * 0.8f) + backgroundPaddingLeft * 2, MeasureSpec.EXACTLY);
                    } else {
                        widthSpec = MeasureSpec.makeMeasureSpec(isPortrait ? width + backgroundPaddingLeft * 2 : (int) Math.max(width * 0.8f, Math.min(Utilities.dp(480), width)) + backgroundPaddingLeft * 2, MeasureSpec.EXACTLY);
                    }
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            if (layoutCount == 0 && startAnimationRunnable != null) {
                Utilities.cancelRunOnUIThread(startAnimationRunnable);
